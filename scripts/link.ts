...
let targetDir = "C:/Users/llej/Documents/SiYuan2/SiYuan/data/widgets";
//********************************************************************************************
...
async function getSiYuanDir() {
  ...
  try {
    ...
    if (response.ok) {
      conf = (await response.json()) as any;
    } else {
...
async function chooseTarget(workspaces) {
  ...
  if (count == 1) {
    ...
    let index: string = await new Promise((resolve, reject) => {
...
async function main() {
  ...
  if (targetDir === "") {
    ...
    if (res === null || res === undefined || res.length === 0) {
      ...
      if (env !== undefined && env !== null && env !== "") {
        ...
        error(
          '\tCan not get SiYuan directory from environment variable "SIYUAN_PLUGIN_DIR", failed!',
        );
        process.exit(1);
      }
    } else {
  ...
  //load plugin.json
  const plugin = JSON.parse(fs.readFileSync("./dist/widget.json", "utf8"));
